...
module.exports = async (req, res) => {
    ...
    try {
      const resp = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 1024,
          system: `You are LawCare AI, a knowledgeable legal assistant for Indian law. Provide practical, accurate information. Always recommend consulting a licensed lawyer. Format with **bold** headers and bullet points.`,
      ...
      if (resp.ok) {
        ...
        const suggested_lawyers = spec ? db.lawyersBySpec(spec) : [];
  ...
  const answer = match?.ans || `Thank you for your question. Under Indian law, this involves several considerations.\n\n**General Advice:**\n• Document everything (dates, communications, receipts)\n• Preserve all relevant agreements and documents\n• Note key timelines carefully\n• Avoid discussing your case without legal counsel\n\nI recommend consulting with one of our specialized lawyers who can review your specific situation.`;
  const suggested_lawyers = match?.spec ? db.lawyersBySpec(match.spec) : [];
